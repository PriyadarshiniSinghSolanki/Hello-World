...
int main(void) {
	...
	printf("Enter your choice:\n"
		...
		case 1: //Add Book Information
			i = 0;
			while (Book[i].accessionNumber) {
				i++;
			}
			Book[i].accessionNumber = i + 1;
		...
		case 2: //Display book information
			...
			int var;
			for (var = 0; var < i; ++var) {
				if (strcmp(Book[var].title, title) == 0) {
					printf("%d %s %s %d %d\n", Book[var].accessionNumber,
							Book[var].title, Book[var].author, Book[var].price,
							Book[var].flag);
					fflush(stdout);
					break;
				}
